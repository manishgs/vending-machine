  ...
  const handleClick = async (e:any) => {
    ...
    try {
      if (selectedProduct) {
        ...
        const res = await axios.post('/api/purchase', { productId: selectedProduct.id, amount: { 1: amount }, vendingMachineId: 1 }, headers);
        setLoading(true);
        setSelectedProduct(defaultProduct);
        setAmount('');
        if (Object.keys(res.data.data.return).length > 0) {
          setMessage(`Please collect your drinks and change ${JSON.stringify(res.data.data.return)}`);
        } else {
